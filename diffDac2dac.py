...
"""  
     ...
     Aim: Transform not so Feflow *_diffDac.dac file into another *_reDac.dac file presen-
          -ting absolute data values instead of data values differences with time step 0  
...
try:
  ...
  print "Usage: dac2diffDac.py FeflowFileName_diffDac.dac"
  sys.exit(1)
...
"""
output_file=input_file[:-4]+'_reDac.dac'
...
while line != '': 
  ...
  #            print "Different number of floats in time step",lineS[1],"(",countFloatT,"versus",countFloat0,"at time step 0)" # check              
            point,init='',''                       # check
